...
- 결과 메모장 저장 -> 엑셀로 저장 
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''
RESULT_PATH = 'C:/GitHub/BM_JTO_Coronavirus/YounghoonKANG/'
...
def crawler(maxpage, query, s_date, e_date):
    ...
    f = open("C:/GitHub/BM_JTO_Coronavirus/YounghoonKANG/contents_naver.txt", 'w', encoding='utf-8')
...
def excel_make():
    data = pd.read_csv(RESULT_PATH + 'contents_naver.txt', sep='\t', header=None, error_bad_lines=False)
    data.columns = ['date', 'newspaper', 'headline', 'content', 'link']
    print(data)

    xlsx_outputFileName = '%s-%s-%s  %s시 %s분 %s초 result.xlsx' % (
        now.year, now.month, now.day, now.hour, now.minute, now.second) # xlsx_name = 'result' + '.xlsx'
